...
namespace Pacman_Zagorschi_Franco
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            string s = "INSERT INTO User1 " +
            "(id_user, Chet, date) VALUES" +
            "('" + Program.id_user + "'," +
            "'" + score.Text.ToString() + "'," +
            "'" + date1 + "')";
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            SqlConnection conn = new SqlConnection(Program.st_connect);
            conn.Open();
            string s = "SELECT Login FROM Avto WHERE id= " + Program.id_user;
            ...
            label253.Text = dataGridView1[0, 0].Value.ToString();
            conn.Close();

            SqlConnection conn1 = new SqlConnection(Program.st_connect);
            conn1.Open();
            string s1 = "SELECT TOP 1(Chet) FROM User1 ORDeR BY Chet DESC ";
            //Делаем запрос к БД через адаптер, что бы потом поместить даные в DATASET 
            SqlDataAdapter adap1 = new SqlDataAdapter(s1, conn1);
            //создаем пустой датасет 
            ...
            label254.Text = dataGridView2[0, 0].Value.ToString();
            conn1.Close();
        }
